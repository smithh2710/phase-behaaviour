function [comp_h, press_h, pressbub_h, pressdew_h] = main(h, h_ref, comp_ref, press_ref, temp, pressc, tempc, acentric, BIP, M_gmol, vt_method, vt_params)
    ...
    try 
        % pressbub_ini_h = pressbubest_multicomp(comp_h, temp, pressc, tempc, acentric);
        pressbub_ini_h = 220e5 ; 
    ...
    % Dew pressure at h
    try 
        % pressdew_ini_h = press_h;
        pressdew_ini_h = 220e5 ; 
...
%   vt_type  : VT method number (0-5 or -1 for direct input)
...
%   -1 - Direct c input
    ...
    % Check if vt_method is a numeric vector (direct c input)
    if isnumeric(vt_method) && length(vt_method) == n
        vt_type = -1;
        vt_opts.c_direct = vt_method(:);
        return;
    end
